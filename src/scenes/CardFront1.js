class CardFront1 extends Phaser.Scene {
    constructor() {
        super('cardFrontScene1')
    }
    ...
    create() {
        ...
        this.flipButton.on('pointerdown', () => {
            //do interaction
            this.scene.start('cardFrontScene2')
        })
        this.flipButton.setVisible(false)


        //create the actual story text
        const story = "Dear Leigh,\n\n I went on a walk the other day and saw a turtle. The turtle stopped to ______ in the middle of the field. Then suddenly, without warning, it started to ______ towards the sky. Finally it started to ______ until the stars came out."
        this.add.text(width/2, height/2-50, story, {
            fontSize: '32px',
        ...
        this.slotPositions.forEach((pos, index) => {
            let zone = this.add.zone(pos.x, pos.y, 120, 30).setRectangleDropZone(120, 50)
            zone.setData('slotIndex', index)
            
            // Visual outline for liningup/testing
            //this.add.graphics().lineStyle(2, 0xffffff).strokeRect(pos.x - 60, pos.y - 25, 120, 50)
        })
